...
class Detector(torch.nn.Module):
    def __init__(self, in_channels: int = 3, num_classes: int = 3):
        ...
        # Define buffers for input normalization
        self.register_buffer("input_mean", torch.as_tensor(INPUT_MEAN))
        self.register_buffer("input_std", torch.as_tensor(INPUT_STD))

        # Down-sampling layers
        self.down1 = self.conv_block(in_channels, 16)   # (B, 3) -> (B, 16, 48, 64)
        self.down2 = self.conv_block(16, 32)            # (B, 16) -> (B, 32, 24, 32)
        self.down3 = self.conv_block(32, 64)            # (B, 32) -> (B, 64, 12, 16)
        self.down4 = self.conv_block(64, 128)           # (B, 64) -> (B, 128, 6, 8)
        self.down5 = self.conv_block(128, 256)          # (B, 128) -> (B, 256, 3, 4)

        # Up-sampling layers
        self.up1 = self.upconv_block(256, 128)          # (B, 256) -> (B, 128, 6, 8)
        self.up2 = self.upconv_block(128 + 128, 64)     # (B, 128+128) -> (B, 64, 12, 16)
        self.up3 = self.upconv_block(64 + 64, 32)       # (B, 64+64) -> (B, 32, 24, 32)
        self.up4 = self.upconv_block(32 + 32, 16)       # (B, 32+32) -> (B, 16, 48, 64)
        self.up5 = self.upconv_block(16 + 16, 16)       # (B, 16+16) -> (B, 16, 96, 128)

        # Output layers
        self.logits = nn.Conv2d(16, num_classes, kernel_size=1)  # Output segmentation map
        self.depth = nn.Conv2d(16, 1, kernel_size=1)             # Output depth map
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        up4_out = torch.cat([up4_out, down1_out], dim=1)  # Concatenate with the added skip connection
